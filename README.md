...
-   采用Maven项目管理，模块化，导入IDE时直接选定hongye-parent的pom导入
    
-   创建数据库hongye，数据库编码为UTF-8，执行hongye.sql文件，初始化数据
    
-   修改application-dev.properties，更新MySQL账号和密码
    
-   Eclipse、IDEA运行SpringbootApplication.java，则可启动项目。或在hongye-parent目录下运行命令mvn clean package，然后在hongye-web/target目录下运行java -jar hongye-web.jar命令
